<?php
		...
		$SQL = "SELECT * FROM periodassignment WHERE SubjectID = 'ASS' OR SubjectID='CPL'"; //Check if ASS or CPL has been inserted into
			$result = mysql_query($SQL);
      $num_rows = mysql_num_rows($result);
                       
           		if($num_rows==0) {
           			header('Location: AssCplWarning.php');
           		}
		$InitializeMusic = "DELETE from periodassignment WHERE SubjectID='MUS'";
			$InitializeMusicResult = mysql_query($InitializeMusic); 
		$InitializeMusicAssignment = "DELETE from assignment WHERE SubjectID='MUS'";
			$InitializeMusicAssignmentResult = mysql_query($InitializeMusicAssignment);
?>
